...
static void init(void)
{
  ...
  status = CAMERA_RECEIVER_Init(&cameraReceiver, &cameraConfig, bufferReady, NULL);
  PRINTF("CAMERA_RECEIVER_Init Status = %d\r\n", status);  //pf

#if ( DEMO_CAMERA == DEMO_CAMERA_RM68191 || DEMO_CAMERA == DEMO_CAMERA_RM68200 || DEMO_CAMERA == DEMO_CAMERA_OV5640 )
  BOARD_InitMipiCsi();
  ...
  PRINTF("CAMERA_DEVICE_Start Status = %d\r\n", status);  //pf
